![Схема компонентов системы](docs/social_banner.png)

# СтудХак

[![status-badge](https://woodpecker.dev.nachert.art/api/badges/13/status.svg)](https://woodpecker.dev.nachert.art/repos/13)

СтудХак — платформа для поиска команд, разработчиков и карьерных мероприятий в хакатон-экосистеме.
...
- `studhack-ui` — Angular 21 SPA с Taiga UI, OAuth2/OIDC-авторизацией и поддержкой `mock` и `real` API
- `studhack-api` — ASP.NET Core 8 API с PostgreSQL, EF Core 8 и background сервисом отправки email
- `docs` — схема базы данных и общая архитектура системы
